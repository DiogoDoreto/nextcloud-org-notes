# Spec: org-content-styles

## Purpose

Defines the visual styling requirements for all HTML elements rendered inside the org viewer content area. All styles use Nextcloud CSS custom properties for theme compatibility and logical CSS properties for RTL support.
...
`#+TITLE:` keywords SHALL be rendered as a prominent `<h1 class="org-title">` element, styled larger than section headings and separated from the document body with a bottom border.

#### Scenario: Title is rendered
- **WHEN** an org file contains `#+TITLE: My Document`
- **THEN** "My Document" renders as a large, bold heading at the top of the content

#### Scenario: Title is visually distinct from section headings
- **WHEN** an org file contains both `#+TITLE:` and `* Heading` entries
- **THEN** the title is visually larger and separated by a border from the section headings
...
Definition lists (`dl`) SHALL render as a two-column grid with bold terms (`dt`) in the first column sized to content and descriptions (`dd`) filling the remaining width, with a `2ch` column gap.

#### Scenario: Term is bold
- **WHEN** an org file contains `- term :: description`
- **THEN** "term" renders in bold

#### Scenario: Term and description are aligned in columns
- **WHEN** an org file contains multiple `- term :: description` entries
- **THEN** all terms align in one column and all descriptions align in a second column

### Requirement: Blockquote styling
Blockquotes SHALL display a left border using `--color-border-dark` and muted text color using `--color-text-maxcontrast`.
...
Links (`a`) inside the content area SHALL be underlined and colored with `--color-primary-element` to distinguish them from body text.

#### Scenario: Links are underlined
- **WHEN** an org file contains `[[https://example.com][link text]]`
- **THEN** "link text" renders as an underlined hyperlink in the primary color
...
`pre.verse` blocks SHALL preserve whitespace and line breaks but render with a body font (not monospace), with an italic style and a left border accent using `--color-border-dark`.
...
### Requirement: Todo keyword styling
`span.todo-keyword` elements SHALL render as pill badges: bold, uppercase, with `--border-radius` corners and colored backgrounds. `TODO`/`FIXME` keywords SHALL use `--color-warning` background with `--color-warning-text` foreground. `DONE` SHALL use `--color-success` background with `--color-success-text` foreground. Unrecognized keywords SHALL use `--color-background-dark` background with `--color-text-maxcontrast` foreground.

#### Scenario: TODO keyword renders as warning pill
- **WHEN** an org heading has a `TODO` keyword
- **THEN** the keyword renders as a pill badge in the Nextcloud warning color

#### Scenario: DONE keyword renders as success pill
- **WHEN** an org heading has a `DONE` keyword
- **THEN** the keyword renders as a pill badge in the Nextcloud success color

#### Scenario: Unknown keyword renders as neutral pill
- **WHEN** an org heading has a custom keyword like `WAIT`
- **THEN** the keyword renders as a muted pill badge
